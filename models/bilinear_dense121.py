...
class BDense121_fc_Attention(nn.Module):
    def __init__(self, num_classes):
        super(BDense121_fc_Attention, self).__init__()
        # 使用DenseNet121的features部分作为卷积层
        self.conv = models.densenet121(weights=models.DenseNet121_Weights.DEFAULT).features
        # 定义注意力机制层
...
class BDense121_all_Attention(nn.Module):
    def __init__(self, num_classes):
        super(BDense121_all_Attention, self).__init__()
        # 使用DenseNet121的features部分作为卷积层
        self.conv = models.densenet121(weights=None).features
        # 定义注意力机制层
        self.attention = AttentionBlock(1024)
...
class BDense121_all_CBAM(nn.Module):
    def __init__(self, num_classes):
        super(BDense121_all_CBAM, self).__init__()
        # 使用DenseNet121的features部分作为卷积层
        self.conv = models.densenet121(weights=None).features
        # 定义注意力机制层
        self.attention = CBAM(1024)
